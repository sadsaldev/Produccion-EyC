...
class getInspections {
    ...
    public function mainDataManager($postopc, $postinitialdate, $postfinaldate){ 
    //Este método es invocado cuando se reciben los datos del formulario

        $fecha_inicio = date('Y-m-d', strtotime($postinitialdate));
        $fecha_final  = date('Y-m-d', strtotime($postfinaldate));
        $opcion = $postopc;
