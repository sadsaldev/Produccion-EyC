...
export const NavProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        if (!['/profile', '/queries'].includes(activeRoute)) {
            ...
            // Si vamos a Perfil directamente desde Login, no clonamos el contenido
            const previousRoute = navigationHistory[navigationHistory.length - 2];
            if (previousRoute && ['/login'].includes(previousRoute)) {
                localStorage.removeItem('backgroundContent');
